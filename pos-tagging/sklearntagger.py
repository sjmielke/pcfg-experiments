...
for (luc, llc) in [("FRENCH", "French"), ("KOREAN", "Korean")]:
    ...
    def read_prepared_corpus(tt):
        ...
        sec = tt
        with open(pref + sec + ".yield") as lex,\
             open(pref + sec + ".posyield") as tag:
    ...
    for n in [40472]:
        ...
        with open(outpref + f'dev.sklearn_tagged.{n}.word', 'w') as fw,\
             open(outpref + f'dev.sklearn_tagged.{n}.pred', 'w') as ft,\
             open(outpref + f'dev.sklearn_tagged.{n}.gold', 'w') as fg:
            right = 0
            wrong = 0
